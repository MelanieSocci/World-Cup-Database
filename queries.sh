...
$PSQL "SELECT DISTINCT name FROM teams INNER JOIN games ON teams.team_id = games.winner_id OR teams.team_id = games.opponent_id WHERE round='Eighth-Final' AND year=2014 ORDER BY name;"

echo -e "\nList of unique winning team names in the whole data set:"
$PSQL "SELECT DISTINCT name FROM games JOIN teams ON games.winner_id = teams.team_id ORDER BY name;"
